...
func NewLiteHandlers(services *service.Services, repos *repository.Repositories, cfg *config.Config, logger *zap.SugaredLogger) *Handlers {
	return &Handlers{
		...
		Admin: &AdminHandler{adminDeps: &adminDeps{
		...
		Metadata:    &MetadataHandler{metadataService: services.Metadata, streamService: services.Stream, logger: logger},
		...
		WS:          NewWSHandler(services.WSHub, logger, wsAllowedOrigins(cfg)...),
